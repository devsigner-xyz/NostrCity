...
In this repository, backend work is primarily the Fastify BFF under `server/src`. Use `./.agents/skills/fastify-best-practices/SKILL.md` for Fastify routes, plugins, validation, hooks, errors, and testing patterns. Use `./.agents/skills/nostr-specialist/SKILL.md` when endpoints touch Nostr event semantics, relay behavior, auth challenges, signing, zaps, WebLN, or NWC flows. Use `./.agents/skills/privacy-and-sensitive-data/SKILL.md` for storage, logs, telemetry, secrets, auth/session data, wallet data, and privacy/security claims.
...
- **config-expert**: Environment variables, secret names, and runtime config validation
...
- Use the `nostr-specialist` skill before changing Nostr-backed contracts, relay assumptions, auth flows, or wallet/zap boundaries
- Coordinate with **security-auditor** and use `privacy-and-sensitive-data` before adding logs, persisted sensitive data, telemetry, or auth/session behavior
...
- Partner with **cicd-engineer**, **docker-specialist**, and **monitoring-architect** for deployment and runtime infrastructure requirements
