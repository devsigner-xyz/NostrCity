...
test('landing muestra manifiesto y CTA principal', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { level: 1, name: /Explora Nostr como ciudad de código abierto/i })).toBeVisible();
  const primaryCta = page.getByRole('link', { name: 'Entrar a la aplicación' }).first();
  await expect(primaryCta).toBeVisible();
  await expect(primaryCta).toHaveAttribute('href', '/app/');
  await expect(page.getByRole('link', { name: 'Documentación' }).first()).toHaveAttribute('href', '/docs/');
});

test('landing incluye seccion para usuarios nostr y filosofia no comercial', async ({ page }) => {
  await page.goto('/');

  await expect(page.getByRole('heading', { name: /Funciones para explorar Nostr/i })).toBeVisible();
  await expect(page.getByText(/sin ánimo de lucro/i).first()).toBeVisible();
