...
| `.opencode/agents` | Versioned project-specific OpenCode subagents. |
| `.agents/skills` | Versioned project-local skills. Prefer these before generic skills when relevant. |
...
- Use `privacy-and-sensitive-data` whenever work touches browser storage, cookies, Playwright `storageState`, logs, traces, screenshots, videos, HARs, telemetry, analytics, browser permissions, secrets, auth tokens, Nostr keys, NWC/WebLN wallet data, DMs, zaps, payment metadata, or privacy/security claims.
- Treat Nostr identity material, `nsec`, `ncryptsec`, NWC connection URIs/secrets, NIP-46 tokens, WebLN wallet details, zap/payment metadata, auth tokens, session IDs, relay-auth challenges, DMs, user profile/contact data, and user-provided relay lists as sensitive.
- Do not log, screenshot, trace, commit, upload, or persist sensitive data unless explicitly required, minimized, documented, and tested.
- Browser persistence includes `localStorage`, `sessionStorage`, IndexedDB, Cache Storage, cookies, and Playwright `storageState`. New persisted keys need an owner, schema/version, data classification, migration or deletion behavior, and tests.
- Do not store raw private keys, NWC secrets, bearer tokens, session cookies, DMs, decrypted wallet data, invoices, preimages, or payment metadata in browser storage without an approved threat model and explicit user consent.
- Use allowlisted structured logs. Redact authorization headers, cookies, tokens, private keys, NWC/NIP-46 secrets, invoices, wallet payloads, DMs, and user-provided relay URLs.
- Keep Playwright screenshots, videos, traces, HARs, and `storageState` failure-scoped, sanitized, short-retention, and generated only with synthetic identities and fake wallet data for sensitive flows.
- Do not add third-party analytics or telemetry without explicit approval. Do not send stable Nostr or wallet identifiers unless justified and documented.
- Browser permissions such as clipboard, geolocation, camera, microphone, and pointer lock require explicit user action, a clear purpose, fallback behavior, and a way to clear stored state.
- Claims such as "private", "secure", "local-only", "encrypted", or "no tracking" must be verified against implementation and documented with current limitations.
...
- Use the `nostr-specialist` skill for Nostr-related work, including relays, NIP-05, NIP-07, NIP-46, NWC, WebLN, zaps, feeds, DMs, follows, and auth.
- Use `privacy-and-sensitive-data` alongside `nostr-specialist` whenever Nostr work touches keys, signers, wallet connections, DMs, zaps, payment metadata, relay lists, auth challenges, persistence, logs, telemetry, or test artifacts.
...
- Treat browser storage keys as persisted data. Keep versioned storage migrations explicit and tested.
...
- Confirm privacy-sensitive changes have storage/log/artifact/telemetry handling documented and tested.
...
- `AGENTS.md`, `.agents/skills`, and `.opencode/agents` are versioned project guidance. Keep them in sync when workflows, skills, or agent relationships change.
...
- Use `security-auditor` for privacy-sensitive changes, auth/session changes, CORS/CSP, secrets, logging, telemetry, browser storage, dependency risk, or test-artifact exposure.
- Use `config-expert` for environment variables, secret names, `.env.example`, runtime config validation, CI secrets, and deployment variables.
- Use `dependency-manager` for package updates, new dependencies, audit findings, bundle/security risk, and lockfile changes.
...
| Privacy, sensitive data, logs, telemetry, browser storage, artifacts | `privacy-and-sensitive-data`, `security-auditor`, `config-expert`. |
...
| CI/CD, GitHub Actions, deployment, Railway | `github-actions-templates`, `use-railway`, `cicd-engineer`, `docker-specialist`, `config-expert`. |
| Git commits, branching, PR workflow | `git-commit`, `git-strategist`, `finishing-a-development-branch`. |
| WebSockets or realtime behavior | `websocket-engineer`, `websocket-architect`. |
| Dependencies and supply chain | `dependency-manager`, `security-auditor`. |
...
- Key agents include `project-orchestrator`, `frontend-specialist`, `backend-architect`, `fullstack-developer`, `auth-architect`, `api-designer`, `schema-validator`, `database-engineer`, `i18n-specialist`, `unit-test-generator`, `integration-test-builder`, `test-architect`, `e2e-test-automator`, `code-reviewer`, `error-detective`, `security-auditor`, `accessibility-auditor`, `cicd-engineer`, `config-expert`, `dependency-manager`, `monitoring-architect`, `performance-profiler`, `seo-optimizer`, `docker-specialist`, `git-strategist`, `websocket-architect`, `refactoring-expert`, and `tech-writer`.
...
- Use `privacy-and-sensitive-data` as the project-specific source of truth for storage, logs, telemetry, artifacts, secrets, browser permissions, and privacy/security claims.
