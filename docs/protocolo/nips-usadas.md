...
| --- | --- | --- | --- |
| NIP-29 | Parcial | Grupos basados en relays, identificados por `{relay,id}`. La app consulta relays de grupos configurados, muestra metadata `kind:39000` firmada por la clave `self` del relay NIP-11, listas disponibles y acciones básicas de escritura. | El estado del grupo depende del relay que lo hospeda y de sus reglas. No cubre todos los flujos de administración, moderación o configuración avanzada de grupos. |
| NIP-51 / `kind:10009` | Parcial | Lista pública de grupos guardados por una persona. Guardar un grupo publica etiquetas `group` y etiquetas `r` para los relays de esos grupos; la sincronización explícita añade también etiquetas `r` con relays de grupos configurados. | Es una lista pública de marcadores y relays; no prueba membresía ni permiso de escritura, y puede quedar replicada después de publicarse. |

Los relays de grupos son una configuración local por defecto. Añadir relays sugeridos en `/groups` o en `/relays` permite descubrir grupos NIP-29 sin publicar `kind:10009`. Guardar un grupo firma y publica la lista con ese grupo y su relay; la sincronización pública explícita publica también los relays de grupos configurados.
