...
| --- | --- | --- | --- |
| NIP-29 | Parcial | Grupos basados en relays, identificados por `{relay,id}`. La app consulta relays de grupos configurados, permite navegación centrada en el relay, muestra metadata `kind:39000`, mensajes básicos `kind:9`, solicitudes de unión/salida `kind:9021`/`kind:9022` e invitaciones con código opcional. El descubrimiento de metadata pública `kind:39000` puede pasar por el BFF para reutilizar caché y verificación. | El estado del grupo depende del relay que lo hospeda y de sus reglas. Sin NIP-11 `self` verificable, la metadata puede mostrarse como no verificada y no se confían administradores, miembros o roles. No cubre todos los flujos de administración, moderación o configuración avanzada. |
