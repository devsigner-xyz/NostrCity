...
Use `privacy-and-sensitive-data` before adding logs, telemetry, request/response body capture, auth/session handling, cookies, secrets, wallet data, DMs, payment metadata, or support diagnostics. Prefer allowlisted structured logs with redaction; never log request or response bodies by default.
...
// Request logging: keep fields allowlisted and never log bodies/secrets by default.
