...
Use `privacy-and-sensitive-data` before configuring screenshots, videos, traces, HARs, `storageState`, browser permissions, auth flows, Nostr keys, wallet data, DMs, or payment flows. Keep sensitive-flow artifacts failure-scoped, sanitized, short-retention, and synthetic-data only.
