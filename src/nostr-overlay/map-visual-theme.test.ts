...
describe('map visual dark theme styles', () => {
    ...
    test('defines dark styles for map zoom controls', () => {
        ...
        expect(styles).toMatch(/\.dark\s+\.nostr-map-options-button\s*\{[^}]*background:\s*color-mix\(in\s+oklab,\s*var\(--background\)\s+92%,\s*transparent\)/s);
        expect(styles).toMatch(/\.dark\s+\.nostr-map-zoom-level\s*\{[^}]*color:\s*var\(--muted-foreground\)/s);
        expect(styles).not.toMatch(/\.dark\s+\.nostr-map-(?:zoom-group|options-button)\s*\{[^}]*rgba\(6,\s*41,\s*31,/s);
    ...
    test('keeps map control surfaces on neutral system tokens', () => {
        ...
        expect(styles).toMatch(/\.nostr-map-options-button\s*\{[^}]*background:\s*color-mix\(in\s+oklab,\s*var\(--background\)\s+92%,\s*transparent\)/s);
        expect(styles).not.toMatch(/\.nostr-map-theme-toggle-group/);
    });

    test('uses standard bordered map control wrappers', () => {
        expect(styles).toMatch(/\.nostr-map-zoom-group\s*\{[^}]*border:\s*1px\s+solid\s+var\(--border\)/s);
        expect(styles).toMatch(/\.nostr-map-options-button\s*\{[^}]*border:\s*1px\s+solid\s+var\(--border\)/s);
    });

    test('aligns map options with map action control surfaces', () => {
        expect(styles).toMatch(/\.nostr-map-options-button\s*\{[^}]*width:\s*38px;[^}]*height:\s*38px;/s);
        expect(styles).toMatch(/\.dark\s+\.nostr-map-options-button\s*\{[^}]*background:\s*color-mix\(in\s+oklab,\s*var\(--background\)\s+92%,\s*transparent\)/s);
    });

    test('separates zoom controls and keeps zoom buttons circular', () => {
        expect(styles).toMatch(/\.nostr-map-zoom-group\s*\{[^}]*gap:\s*0\.25rem;/s);
        expect(styles).toMatch(/\.nostr-map-zoom-button\s*\{[^}]*border-radius:\s*999px\s*!important;/s);
        expect(styles).toMatch(/\.nostr-map-zoom-level\s*\{[^}]*border:\s*0;/s);
    });

    test('does not keep legacy display toggle styles after moving layers into the menu', () => {
        expect(styles).not.toMatch(/\.nostr-map-display-toggle-button/);
        expect(styles).not.toMatch(/\.nostr-map-display-toggle-group/);
    });
